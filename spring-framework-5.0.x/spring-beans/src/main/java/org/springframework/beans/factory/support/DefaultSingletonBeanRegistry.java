...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	protected void addSingletonFactory(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			if (!this.singletonObjects.containsKey(beanName)) {
				// 先将indexDao1 的bean放入singletonFactories中, 之后在创建indexDao2实例调用
				// getSigton方法的时候 将从singletonFactories在把这个indexDao1 的bean拿出来
				this.singletonFactories.put(beanName, singletonFactory);
				//  这个没有用到（或者我不知道哪里用到了）， 是在new indexDao1的bean实例的时候调用getSigton方法放进去的（但是第一次是不会进放进去的逻辑，所以也不知道是怎么放进去的）
				// indexDao1的时候 earlySingletonObjects的size为0
				this.earlySingletonObjects.remove(beanName);
				// 这个也不知道哪里用到了
				this.registeredSingletons.add(beanName);
			}
	...
	protected Object getSingleton(String beanName, boolean allowEarlyReference) {
		Object singletonObject = this.singletonObjects.get(beanName);// singletonObjects是一级缓存map 单例池
		// 这里判断bean实例是否能被拿到
		// 如果没拿到返回null，在判断this.inCreationCheckExclusions是否存在这个beanName
		...
		// 之后这里就将dao1返回出了，dao2拿到dao1实例完成依赖， 完成dao2的整个实力化过程，然后回到最初的调用链dao1，dao1也就拿到了dao2
		// 完成dao1的整个实例化过程！结束

		// earlySingletonObjects（三级缓存）的用处是 filed属性类型匹配的时候用了一下（假设我们dao2依赖的dao1是 dao1 实例的代理对象，这个三级缓存就是解决这个问题的），
		// 看AutowiredAnnotationBeanPostProcessor中的注释
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) { // 拿不到就开始创建对象
